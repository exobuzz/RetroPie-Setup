...
function section_gui_setup() {
    ...
    case "$section" in
        all|inst)
            ...
            for id in "${__mod_id[@]}"; do
                ...
                ! [[ "${__mod_info[$id/section]}" =~ ^(depends|)$ ]] && ids+=("$id")
            done
            ;;
         *)
    ...
    while true; do
        ...
        if [[ "$has_net" -eq 1 && "$section" != "driver" && "$section" != "depends" ]]; then
            options+=(
                I "Install all $name" "This will install all $name. If a package is not installed, and a pre-compiled binary is available it will be used. If a package is already installed, it will be updated by the method used previously."
                X "Remove all installed $name" "X This will remove all installed $name."
            )
        fi
